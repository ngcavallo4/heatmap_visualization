...
class Plotter():
    """
        ...
        rotate: :class:`int`
            Integer multiple of 90 indicating rotation of plotted interpolation.
    ...
    def plot_heatmap(self, path: os.PathLike, file: str, match_steps: bool, gpregressor: GPRegressor, match_scale: bool = False, transparent: dict = None, optimizer: bool = False, latlon: bool = False):
        ...
        self.csvparser = CSVParser(path, file)
    
        self.latlon = latlon
        self.plot_legs(match_steps, gpregressor, match_scale, transparent, optimizer)

    def plot_legs(self, match_steps: bool, gpregressor: GPRegressor, match_scale: bool, transparent: dict, optimizer: bool):
    ...
    def worker_func(self, request: str, match_steps: bool, gpregressor: GPRegressor, optimizer: bool, results: dict):
        ...
        axis_index = self.leg_list.index(request)

        return (z_pred, var, x, y, stiff, title, x_range, y_range, axis_index, grid)
    ...
    def initialize_subplots(self):
            ...
            if len(self.leg_list) > 1:
                if 'all' not in self.leg_list:
                    ...
                    ncols = len(self.leg_list)
            elif len(self.leg_list) == 1:
                ncols = 1

            self.fig, self.axs = plt.subplots(nrows,ncols,figsize=(17,7), layout='tight')
